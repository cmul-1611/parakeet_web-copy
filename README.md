...
| 📦 **Automatic Quantization** | Encoder precision follows the backend automatically: on WebGPU it prefers fp16 when the model repo ships it (~1.2 GB, near-lossless) and otherwise uses fp32 (~2.4 GB); on WASM it uses int8 (smaller, and the only one that fits the browser's 32-bit heap / blob-fetch limit). The decoder runs int8, or fp16 alongside an fp16 encoder. fp16 files are built by `scripts/quantize-fp16.py` (see also the [grikdotnet fp16 model card](https://huggingface.co/grikdotnet/parakeet-tdt-0.6b-fp16) documenting the same conversion) |
