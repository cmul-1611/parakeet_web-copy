...
A served list can carry its own default strength on a `#!strength N` line (N in -10 to 10): loading that list forces the strength slider to N, so a curated list ships pre-tuned. A list can declare its own default augmentation on a `#!augment FLAGS` line (the file-level equivalent of the **Augment phrases** toggle; same flags as `AUG`, e.g. `#!augment faph` or `#!augment s` to force none): it overrides the toggle for that list, while a per-phrase `AUG` field still wins. A list can also override the proclitic prefixes used by the `p` augmentation with a `#!prefixes a' b' ...` line (whitespace-separated); the default is the French elision set (`l'`, `d'`, `L'`, `D'`). A prefix ending in an apostrophe only attaches before a vowel (so `l'amoxicilline` but never `l'beta`); any other prefix (e.g. Arabic `al-`) attaches unconditionally.
