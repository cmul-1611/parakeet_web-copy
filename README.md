...
| 📦 **Automatic Quantization** | Encoder precision follows the backend automatically: on WebGPU it uses fp16 (~1.2 GB, near-lossless, and lighter to serve; on a backend without fp16 compute kernels it is upcast to fp32 at identical accuracy), only falling back to a full fp32 encoder (~2.4 GB) when the model repo ships no fp16 file; on WASM it uses int8 (smaller, and the only one that fits the browser's 32-bit heap / blob-fetch limit). The decoder runs int8, or fp16 alongside an fp16 encoder. fp16 files are built by the `scripts/quantize-fp16.py` script that ships in the [Olicorne/parakeet-tdt-0.6b-v3-smoothquant-onnx](https://huggingface.co/Olicorne/parakeet-tdt-0.6b-v3-smoothquant-onnx) model repo (see also the [grikdotnet fp16 model card](https://huggingface.co/grikdotnet/parakeet-tdt-0.6b-fp16) documenting the same conversion) |
