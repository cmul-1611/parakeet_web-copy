...
| File | Role |
|---|---|
| `audio.js` | Shared audio helpers: PCM resample to 16 kHz (`resamplePcmTo16k`), an RMS level monitor (`createLevelMonitor`), and `buildRecordingRateCandidates` (the ordered sample-rate list for opening the recording AudioContext: native/reported rate and the browser default before the SpeechMike-specific low rates, so a Firefox mic that reports no rate is not forced into a 16 kHz context and slowed down). Used by both local and remote recording. `buildRecordingRateCandidates` unit-tested in `test/unit/recording-rate-candidates.test.mjs`. |
...
| `test/e2e/transcription-int8-lite.spec.js` | **Tier 3** in-browser proof that the opt-in **int8 lite** encoder (`encoder-model.int8.lite.onnx`, ~757 MB) loads and transcribes on WASM, fetching the lite file (not the default int8). SELF-SKIPS when the lite encoder is not served (upstream ships none; the Olicorne repo does). |
