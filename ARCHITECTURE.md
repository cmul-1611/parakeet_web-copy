...
| `grid_search_benchmark.mjs` | Grid-search WER bench over NeMo jsonl manifest(s): reuses the production decode + phrase-boost trie unchanged and sweeps encoder-quant x decoder-quant x beam-width x boost-strength (`--quant int8,fp16,fp32` benchmarks each encoder quant, the outer dimension, with its own model load + encoder cache), printing WER/Levenshtein per combination. `--decoder-quants int8,fp16,fp32` (default fp32) sweeps the fused decoder_joint quant independently of the encoder quant, nested under each encoder quant so the cached encoder output is reused across the decoder sweep (no re-encode); the accuracy table gains a `dec` column. Sorts by CER by default; multi-dataset overall is size-weighted (micro-average). When the decoder runs with `collectBeamStats`, the table also reports the per-step beam expansion width (`hyp_med`/`hyp_max`/`steps`, the CPU-cost driver behind the beam-on-CPU question in murmure#338) and the 5-min system load (`load5`) at the end of each cell plus a per-run average, so a cell whose decode timing was inflated by an unrelated process is visible. |
