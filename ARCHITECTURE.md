...
| `test/e2e/fleurs-chunking.spec.js` | **Tier 3** realistic long-audio path: feeds the ~3 min stitched FLEURS clip at the (unseeded) default window, exercising the backend-aware int8-safe default (`defaultChunkDurationForBackend`) so the clip splits into ~a dozen chunks; asserts chunking engaged, content recovered, and no runaway seam duplication. |
