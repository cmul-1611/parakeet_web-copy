...
| `grid_search_benchmark.mjs` | Grid-search WER bench over NeMo jsonl manifest(s): reuses the production decode + phrase-boost trie unchanged and sweeps quant x beam-width x boost-strength (`--quant int8,fp16,fp32` benchmarks each quant, the outer dimension, with its own model load + encoder cache), printing WER/Levenshtein per combination. Sorts by CER by default; multi-dataset overall is size-weighted (micro-average). |
