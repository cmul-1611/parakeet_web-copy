...
| 📦 **Quantification automatique** | La précision de l'encodeur suit automatiquement le backend : fp32 sur WebGPU (précis), int8 sur WASM (plus léger, requis par la limite de récupération de blob du navigateur) ; le décodeur tourne toujours en int8 |
...
Lorsque `LOCAL_MODEL_PATH` est défini, le conteneur encode chaque liste fournie par l'opérateur en identifiants de tokens au démarrage afin que les navigateurs des visiteurs sautent ce travail. Cet encodage s'exécute tout de même à **chaque** démarrage du conteneur, ce qui est lent pour une très grande liste (10k à 100k phrases). Pour l'éviter, précompilez la liste une fois :
