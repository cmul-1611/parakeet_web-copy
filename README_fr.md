...
| 📦 **Quantification automatique** | La précision de l'encodeur suit automatiquement le backend : sur WebGPU, il préfère fp16 quand le dépôt du modèle le fournit (~1,2 Go, quasi sans perte) et utilise sinon fp32 (~2,4 Go) ; sur WASM, il utilise int8 (plus léger, et le seul qui tienne dans le tas 32 bits du navigateur / la limite de récupération de blob). Le décodeur tourne en int8, ou en fp16 aux côtés d'un encodeur fp16 |
