...
| 📦 **Quantification automatique** | La précision de l'encodeur suit automatiquement le backend : sur WebGPU, il préfère fp16 quand le dépôt du modèle le fournit (~1,2 Go, quasi sans perte) et utilise sinon fp32 (~2,4 Go) ; sur WASM, il utilise int8 (plus léger, et le seul qui tienne dans le tas 32 bits du navigateur / la limite de récupération de blob). Le décodeur tourne en int8, ou en fp16 aux côtés d'un encodeur fp16. Les fichiers fp16 sont générés par `scripts/quantize-fp16.py` (voir aussi la [fiche du modèle fp16 de grikdotnet](https://huggingface.co/grikdotnet/parakeet-tdt-0.6b-fp16) qui documente la même conversion) |
...
- **[istupakov/parakeet-tdt-0.6b-v3-onnx](https://huggingface.co/istupakov/parakeet-tdt-0.6b-v3-onnx)** – Conversion ONNX du modèle
  - Cela a été essentiel pour me permettre de réaliser ma propre quantification améliorée, disponible sur [Olicorne/parakeet-tdt-0.6b-v3-smoothquant-onnx](https://huggingface.co/Olicorne/parakeet-tdt-0.6b-v3-smoothquant-onnx)
