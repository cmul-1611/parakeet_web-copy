...
//
// The fixture two-speakers.mp3 is JFK's English excerpt (~11 s) followed by a
// French clinical clip (~4 s), two acoustically very different voices, so the
// diarizer must split it into at least two speakers and the first turn's speaker
// must differ from the last turn's.
//
...
test('diarizes a two-speaker clip into colour-coded speaker turns (WASM)', async ({ page, request, baseURL }) => {
  ...
  const FIXTURE_AUDIO = fixture('two-speakers.mp3');
