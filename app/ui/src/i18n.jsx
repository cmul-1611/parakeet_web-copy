...
const translations = {
  en: {
    ...
    tooltipBoost: 'Bias transcription toward (or away from) words or phrases you list here (names, jargon, drug names, acronyms). One phrase per line; add an optional per-phrase weight after a colon, e.g. "acetaminophen:2.5" to boost or "um:-3" to penalise. Add a second number to set a per-phrase top-k gate, e.g. "venlafaxine:5:50": the phrase is only nudged when its token is already in the model\'s top 50 candidates, so boosting never forces a word the model did not consider (default top-k 25). Add a third field to augment a single phrase into extra surface forms: any mix of "f" (Title Case), "a" (ALL CAPS), "p" (proclitic prefixes like l\'/d\') and "h" (strip symbols, e.g. "alpha-methyl" also boosts "alpha methyl"), e.g. "amoxicilline:5::faph"; "s" forces none and "i" all of them; leave a field empty to keep its default, e.g. "venlafaxine::50". The default strength slider scales every phrase (negative inverts boosts into penalties, 0 disables); a served list can carry its own default with a "#!strength N" line, which forces the slider when you load that list. Runs 100% on your device. Because decoding is greedy, this is best-effort: it nudges each step toward your phrases but cannot always recover a word the decoder already discarded. Default: empty, strength 1.',
    tooltipBoostStrength: 'A single multiplier applied to every boosted phrase, both the ones you type and any list you load (negative inverts boosts into penalties, 0 disables boosting entirely). When you load a list that carries a "#!strength N" header line, this value is set to that header so the list comes pre-tuned; changing it afterwards still affects all phrases at once, your own and the loaded list alike. Per-phrase weights (e.g. "venlafaxine:5") are scaled by this value too. Default: 1.',
    tooltipBoostAugment: 'The decoder matches exact tokens, and tokens are case-sensitive, so "venlafaxine" alone does not boost "Venlafaxine" or "VENLAFAXINE". When on, each phrase you type is augmented into extra surface forms: Title Case, ALL CAPS, proclitic prefixes (so a vowel-initial term like "amoxicilline" also boosts "l\'amoxicilline" / "d\'amoxicilline"), and symbol-stripped forms (so "alpha-methyl" also boosts "alpha methyl"). This multiplies the number of phrases (a little more memory and a slightly longer rebuild). This checkbox sets the default for every phrase; override a single phrase with a flag suffix: "s" (none), "i" (all), or any mix of "f"/"a"/"p"/"h", e.g. "amoxicilline:5::faph". A served list can set a file-level default with a "#!augment ..." line, or change the prefixes with a "#!prefixes l\' d\' ..." line (default is the French elision set). Default: off.',
  ...
  fr: {
    ...
    tooltipBoost: "Oriente la transcription vers (ou \u00e0 l'\u00e9cart de) les mots ou phrases list\u00e9s ici (noms, jargon, m\u00e9dicaments, acronymes). Une phrase par ligne ; ajoutez un poids optionnel apr\u00e8s deux-points, ex. : \"paracetamol:2.5\" pour renforcer ou \"euh:-3\" pour p\u00e9naliser. Ajoutez un second nombre pour fixer une garde top-k par phrase, ex. : \"venlafaxine:5:50\" : la phrase n'est renforc\u00e9e que si son token figure d\u00e9j\u00e0 parmi les 50 meilleurs candidats du mod\u00e8le, afin de ne jamais forcer un mot que le mod\u00e8le n'a pas envisag\u00e9 (top-k 25 par d\u00e9faut). Ajoutez un troisi\u00e8me champ pour augmenter une phrase en formes suppl\u00e9mentaires : un m\u00e9lange de \u00ab f \u00bb (Casse De Titre), \u00ab a \u00bb (MAJUSCULES) et \u00ab p \u00bb (pr\u00e9fixes proclitiques comme l'/d') et \u00ab h \u00bb (supprime les symboles, ex. : alpha-methyl renforce aussi alpha methyl), ex. : \u00ab amoxicilline:5::faph \u00bb ; \u00ab s \u00bb n'en force aucune et \u00ab i \u00bb toutes ; laissez un champ vide pour garder sa valeur par d\u00e9faut, ex. : \u00ab venlafaxine::50 \u00bb. Le curseur d'intensit\u00e9 par d\u00e9faut multiplie chaque phrase (n\u00e9gatif inverse les renforcements en p\u00e9nalit\u00e9s, 0 d\u00e9sactive)\u00a0; une liste fournie peut porter sa propre valeur par d\u00e9faut via une ligne \u00ab\u00a0#!strength N\u00a0\u00bb, qui force le curseur lorsque vous chargez cette liste. Fonctionne 100\u00a0% sur votre appareil. Le d\u00e9codage \u00e9tant glouton, c'est un effet \"au mieux\"\u00a0: il oriente chaque \u00e9tape vers vos phrases mais ne peut pas toujours r\u00e9cup\u00e9rer un mot d\u00e9j\u00e0 \u00e9cart\u00e9 par le d\u00e9codeur. D\u00e9faut\u00a0: vide, intensit\u00e9 1.",
    tooltipBoostStrength: "Un multiplicateur unique appliqu\u00e9 \u00e0 chaque phrase renforc\u00e9e, aussi bien celles que vous saisissez que celles d'une liste charg\u00e9e (une valeur n\u00e9gative inverse les renforcements en p\u00e9nalit\u00e9s, 0 d\u00e9sactive enti\u00e8rement le renforcement). Lorsque vous chargez une liste qui porte une ligne d'en-t\u00eate \u00ab\u00a0#!strength N\u00a0\u00bb, cette valeur est d\u00e9finie sur celle de l'en-t\u00eate afin que la liste soit pr\u00e9r\u00e9gl\u00e9e\u00a0; la modifier ensuite agit toujours sur toutes les phrases \u00e0 la fois, les v\u00f4tres comme celles de la liste charg\u00e9e. Les poids par phrase (ex.\u00a0: \u00ab\u00a0venlafaxine:5\u00a0\u00bb) sont \u00e9galement multipli\u00e9s par cette valeur. D\u00e9faut\u00a0: 1.",
    tooltipBoostAugment: "Le d\u00e9codeur compare des tokens exacts, et les tokens sont sensibles \u00e0 la casse\u00a0: \u00ab\u00a0venlafaxine\u00a0\u00bb seul ne renforce donc pas \u00ab\u00a0Venlafaxine\u00a0\u00bb ni \u00ab\u00a0VENLAFAXINE\u00a0\u00bb. Lorsque l'option est activ\u00e9e, chaque phrase saisie est augment\u00e9e en formes suppl\u00e9mentaires : Casse De Titre, MAJUSCULES, pr\u00e9fixes proclitiques (ainsi un terme commen\u00e7ant par une voyelle comme \u00ab amoxicilline \u00bb renforce aussi \u00ab l'amoxicilline \u00bb / \u00ab d'amoxicilline \u00bb) et les formes sans symboles (ainsi \u00ab alpha-methyl \u00bb renforce aussi \u00ab alpha methyl \u00bb). Cela multiplie le nombre de phrases (un peu plus de m\u00e9moire et une reconstruction l\u00e9g\u00e8rement plus longue). Cette case d\u00e9finit la valeur par d\u00e9faut pour chaque phrase\u00a0; remplacez-la pour une phrase avec un suffixe\u00a0: \u00ab\u00a0s\u00a0\u00bb (aucune), \u00ab\u00a0i\u00a0\u00bb (toutes) ou un m\u00e9lange de \u00ab\u00a0f\u00a0\u00bb/\u00ab\u00a0a\u00a0\u00bb/\u00ab\u00a0p\u00a0\u00bb/\u00ab\u00a0h\u00a0\u00bb, ex.\u00a0: \u00ab\u00a0amoxicilline:5::faph\u00a0\u00bb. Une liste fournie peut d\u00e9finir une augmentation par d\u00e9faut via une ligne \u00ab\u00a0#!augment ...\u00a0\u00bb, ou changer les pr\u00e9fixes via une ligne \u00ab\u00a0#!prefixes l' d' ...\u00a0\u00bb (par d\u00e9faut, l'\u00e9lision fran\u00e7aise). D\u00e9faut\u00a0: d\u00e9sactiv\u00e9.",
